...
summary_report = pd.read_csv('reportSummary.csv')
    ...
    if user_input.lower() == "exit":
        ...
        if company_input not in ['Apple', 'Microsoft', 'Tesla']:
            ...
            fiscal_year_options = {
                		...
                		"Tesla": [2023, 2022, 2021],
                		"Apple": [2024, 2023, 2022]
            }
            available_years = fiscal_year_options[company_input]
            print(f"\nThe data for the fiscal years {', '.join(map(str, available_years))} is currently available.")
            
            try:
                fiscal_year = int(input("Enter the fiscal year for the selected company: "))
                if fiscal_year not in available_years:
                    print("Invalid fiscal year. Please start the chatbot session again.")
                    break
            except ValueError:
                print("Invalid input. Please enter a valid year by restarting the chatbot session.")
                break
    else:
